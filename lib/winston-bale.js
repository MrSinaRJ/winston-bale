/*
 * winston-bale.js: Transport for outputting logs to Bale messenger
 *
 * (C) 2015 Amir Moeen Rahmani
 * MIT LICENCE
 */
 ...
 * @param {string} options.token - Bale messenger bot authentication token.
 * @param {string} options.chatId - Bale messenger unique identifier for chat.
 * @param {string} [options.messageThreadId=''] - Bale messenger unique identifier of a message thread to which the message belongs.
 * @param {string} [options.parseMode=''] - Bale messenger mode for parsing entities in the message text.
 ...
 * @param {string} [options.name='winston-bale'] - Logger's name.
 ...
 */
module.exports = class Bale extends Transport {
  constructor (options) {
    super(options)
    options = options || {}

    if (!options.token || !options.chatId) {
      throw new Error("winston-bale requires 'token' and 'chatId' property")
    }
    if (options.formatMessage && typeof options.formatMessage !== 'function') {
      throw new Error("winston-bale 'formatMessage' property should be function")
    }
    ...
    this.name = options.name || this.name || 'winston-bale'
  ...
  /**
   * Sends the given message to Bale messenger splitted as needed.
   *
   * @function send
  ...
  /**
   * Actual method that sends the given message to Bale messenger.
   *
